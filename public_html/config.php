...
// Телефон необязателен, но раньше принимался вообще без проверки — можно
// было ввести хоть сотню цифр подряд (users.phone — varchar(20), лишнее
// просто молча обрезалось бы при записи в базу).
define('PHONE_MAX_LENGTH', 20);

/**
 * Проверяет телефон (необязательное поле): допускаются только цифры и
 * стандартные разделители (+, пробел, скобки, дефис), а само число цифр —
 * от 10 до 15 (международный формат E.164). Пустая строка считается
 * валидной — поле необязательное. Возвращает null, если всё в порядке,
 * иначе — текст ошибки для показа пользователю.
 */
function rr_validate_phone($phone) {
    if ($phone === '') {
        return null;
    }
    if (strlen($phone) > PHONE_MAX_LENGTH || !preg_match('/^\+?[0-9\s\-\(\)]+$/u', $phone)) {
        return 'Телефон может содержать только цифры и + ( ) -, не длиннее ' . PHONE_MAX_LENGTH . ' символов';
    }
